...
typedef void (*TaskFuncType)(void *data, int threadIndex, int threadCount, int taskIndex,
                             int taskCount, int taskIndex0, int taskIndex1, int taskIndex2,
...
static void lRunTask(void *ti) {
  ...
  taskInfo->func(taskInfo->data, threadIndex, threadCount, taskInfo->taskIndex,
                 taskInfo->taskCount(), taskInfo->taskIndex0(), taskInfo->taskIndex1(),
                 taskInfo->taskIndex2(), taskInfo->taskCount0(), taskInfo->taskCount1(),
                 taskInfo->taskCount2());
}
...
static void __cdecl lRunTask(LPVOID param) {
  ...
  ti->func(ti->data, threadIndex, threadCount, ti->taskIndex, ti->taskCount(), ti->taskIndex0(),
           ti->taskIndex1(), ti->taskIndex2(), ti->taskCount0(), ti->taskCount1(),
           ti->taskCount2());
...
inline void TaskGroup::Sync() {
  ...
  while (numUnfinishedTasks > 0) {
    ...
    myTask->func(myTask->data, 0, 1, myTask->taskIndex, myTask->taskCount(), myTask->taskIndex0(),
                 myTask->taskIndex1(), myTask->taskIndex2(), myTask->taskCount0(),
...
inline void TaskGroup::Launch(int baseIndex, int count) {
  for (int i = 0; i < count; ++i) {
    ...
    futures.push_back(hpx::async(ti->func, ti->data, threadIndex, threadCount, ti->taskIndex,
